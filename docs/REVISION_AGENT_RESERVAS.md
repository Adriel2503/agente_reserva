...
- **Contexto del orquestador**: id_empresa, id_prospecto, agendar_usuario, agendar_sucursal se reciben y se convierten a int (1/0) en AgentContext; las tools los usan y los pasan a confirm_booking.
...
1. **main.py (chat docstring)**: Se añaden id_prospecto y agendar_sucursal al contexto documentado para que quien integre el orquestador sepa qué enviar.
...
1. Orquestador envía message, session_id, context (config con id_empresa, id_prospecto, agendar_usuario, agendar_sucursal, personalidad, etc.).
...
4. create_booking valida datos, valida horario con ScheduleValidator, llama a confirm_booking con id_prospecto, agendar_usuario, agendar_sucursal, sucursal (nombre), etc.
